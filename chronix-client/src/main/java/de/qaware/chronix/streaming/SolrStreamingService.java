...
import de.qaware.chronix.converter.DocumentConverterCaller;
import de.qaware.chronix.streaming.handler.SolrStreamingHandler;
import de.qaware.chronix.streaming.handler.TimeSeriesHandler;
...
public class SolrStreamingService<T> implements Iterator<T> {
    ...
    private void streamDocumentsFromSolr() {
        ...
        try {
            ...
            do {
                SolrDocument document = solrStreamingHandler.poll();
                if (document != null) {
            ...
            LOGGER.info("Exception while streaming the data points from Solr", e);
        }
    }
}
